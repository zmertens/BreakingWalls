...
void VoronoiPlanet::generateSeeds(int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        float r = sqrt(1 - y * y);
        float phi = fmod(i * increment, 2.0f * PI);
        float x = cos(phi) * r;
        float z = sin(phi) * r;
        m_seedPositions.emplace_back(x, y, z);
    }
}
...
void VoronoiPlanet::uploadToGPU() {
    ...
    struct Vertex { float p[3]; float n[3]; };
    std::vector<Vertex> verts;
    verts.reserve(m_vertices.size());
    for (size_t i = 0; i < m_vertices.size(); ++i) {
        Vertex v;
        v.p[0] = m_vertices[i].x; v.p[1] = m_vertices[i].y; v.p[2] = m_vertices[i].z;
        v.n[0] = m_normals[i].x; v.n[1] = m_normals[i].y; v.n[2] = m_normals[i].z;
        verts.push_back(v);
    }
